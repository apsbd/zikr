...
-- Users can update their own profile (but not role unless they're superuser)
CREATE POLICY "Users can update their own profile" ON user_profiles
    FOR UPDATE USING (
        auth.uid() = user_id AND (
            -- Allow non-role updates
            (OLD.role = NEW.role) OR 
            -- Allow role updates only for superusers
            (OLD.role = 'superuser' AND NEW.role IN ('user', 'admin', 'superuser'))
        )
    );
